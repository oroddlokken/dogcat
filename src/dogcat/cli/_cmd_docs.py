    ...
    def prime(
        ...
        if opinionated:
            opinionated_rules = (
                ...
                "  When creating issues, set appropriate labels"
                " using `--labels`\n"
                "  based on the issue content (e.g. `cli`, `tui`,"
                " `api`, `docs`,\n"
                "  `testing`, `refactor`, `ux`, `performance`,"
                " etc.).\n"
                "  Do NOT batch-mark an entire backlog as"
                " `in_progress` upfront.\n"
                "  Mark each issue `in_progress` right when you"
                " start working on it.\n"
                "  When running multiple `dcat` commands, make"
                " separate parallel\n"
